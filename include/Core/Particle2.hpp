...
class Particle2 {
   ...
   public:
    ...
    Particle2() = delete;
    Particle2(int pdg, int status, int index, int charge, double mass, double px, double py, double pz, double E, double vx, double vy, double vz, double vt, double beta, double chi2pid);
    ...
    auto theta(const Particle2& other) const -> double;
    auto p() const -> double;
...
namespace fmt {
template <>
class formatter<Core::Particle2> {
   public:
    constexpr auto parse(format_parse_context& ctx) { return ctx.begin(); }

    template <typename Context>
    constexpr auto format(Core::Particle2 const& foo, Context& ctx) const {
